...
<div class="max-w-4xl mx-auto">
    ...
    <div class="mb-8">
        <h2 class="text-3xl font-bold text-gray-900">Add Teacher Record</h2>
            ...
            <form action="{{ route('faculty.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="space-y-1">
                        ...
                        <input class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm" id="name" name="name" type="text" value="{{ old('name') }}" required/>
                    </div>
                    ...
                    <div class="space-y-1">
                        ...
                        <input class="w-full px-4 py-3 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-1 focus:ring-blue-500 text-sm" id="email" name="email" type="email" value="{{ old('email') }}" required/>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 border border-gray-300 rounded-lg bg-gray-50 max-h-64 overflow-y-auto">
                        @foreach($subjects as $subject)
                            <label class="flex items-center space-x-3 p-3 bg-white border border-gray-200 rounded-lg hover:border-blue-400 cursor-pointer transition-all">
                                <input type="checkbox" name="subject_ids[]" value="{{ $subject->id }}" class="text-blue-600 focus:ring-blue-500 rounded border-gray-300" {{ in_array($subject->id, old('subject_ids', [])) ? 'checked' : '' }}>
                ...
                <div class="pt-6 flex justify-end">
                    <button class="bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-lg transition-all" type="submit">Save Record</button>
                </div>
            </form>
        </div>
    </div>
